 ...
 *
 * Debugged 2007 by Gerald Roehrbein. Removed memory leak! Added functions required by Watchdog!
 *
 *
 */
...
static struct top_proc **pactive = NULL; /* Init pointer ! */
...
new_proc()
{
	...
	{
            /* Have to free procblock before discarding the pointer! */
            free(procblock);
	    procblock = NULL;
	}
    }
...

{
    ...
    /*
     * Changed by Gerald Roehrbein
     * Next if stops a memory leak!
     *
     */

    if (pactive == NULL ) /* If already initialized than don't allocate this memory again! */
    {
