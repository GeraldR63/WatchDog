...
/*
   (w) 1998 Gerald Roehrbein

   Definiert eine Methode, die ein als Parameter
   uebergebenes directory scannt und fuer jeden
   gefundenen Eintrag die callback routine, die
   als zweiter Parameter angegeben wurde, aufruft!

   Das typedef definiert den Prototypen fuer die
   callback Routine und die
   Funktion dir_do den Prototypen fuer den 
   Funktionsaufruf.
*/
typedef void (*fn)(char*, int);
void dir_do(const char* , const fn, const int sock);
